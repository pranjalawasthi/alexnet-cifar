...
with tf.Session(graph=graph) as session:
    ...
    #summary_writer = tf.summary.FileWriter('C:/Users/pranjal/Downloads/cifar-10', session.graph)
    ...
    for step in range(num_steps):
        ...
        if step % 20 == 0:
          _, l, predictions, summ = session.run(
            [optimizer, loss, train_prediction, summary], feed_dict = feed_dict
            )
